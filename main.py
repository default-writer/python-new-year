...
def cipher(f, next, shift, alphabet, array, sha_alphabet, sha_plaintext):
    shuffle_binb(alphabet, sha_alphabet)
    shuffle_binb(alphabet, sha_plaintext)
    i = 0
    while i < shift:
      array = [*map(f(alphabet, next), array)]
      i = i + 1
    return array
